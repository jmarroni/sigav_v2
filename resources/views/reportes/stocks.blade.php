                    ...
                    <div class="col-sm-12">
                        <table id="tabla_compras">
                            ...
                            <tbody id="tbody">
                                ...
                                <?php 
                                // Debug: Verificar los valores antes del cálculo
                                echo "<!-- DEBUG: Producto ID: " . $producto->id . " -->";
                                echo "<!-- DEBUG: precio_unidad: " . var_export($producto->precio_unidad, true) . " (tipo: " . gettype($producto->precio_unidad) . ") -->";
                                echo "<!-- DEBUG: stockactual: " . var_export($producto->stockactual, true) . " (tipo: " . gettype($producto->stockactual) . ") -->";
                                
                                // Verificar si los valores son numéricos
                                if (!is_numeric($producto->precio_unidad)) {
                                    echo "<!-- ERROR: precio_unidad no es numérico: " . var_export($producto->precio_unidad, true) . " -->";
                                    $producto->precio_unidad = 0;
                                }
                                
                                if (!is_numeric($producto->stockactual)) {
                                    echo "<!-- ERROR: stockactual no es numérico: " . var_export($producto->stockactual, true) . " -->";
                                    $producto->stockactual = 0;
                                }
                                
                                // Convertir a float para asegurar operación numérica
                                $precio = floatval($producto->precio_unidad);
                                $stock = floatval($producto->stockactual);
                                
                                echo "<!-- DEBUG: precio convertido: " . $precio . " -->";
                                echo "<!-- DEBUG: stock convertido: " . $stock . " -->";
                                
                                $subtotal += $precio * $stock;
                                echo "<!-- DEBUG: subtotal acumulado: " . $subtotal . " -->";
                                ?>
