...
if ($_POST){
    ...
    if ($resultado_stock->num_rows > 0) {
        if ($row_stock = $resultado_stock->fetch_assoc()) {
            ...
            $expire = time() + 3600*24;
            setSecureCookie("kiosco", $row_stock["usuario"], $expire);
            setSecureCookie("sucursal", setSucursal($row_stock["sucursal_id"]), $expire);
            setSecureCookie("rol", setRol($row_stock["rol_id"]), $expire);
            if (!(isset($_COOKIE["lista_precio"]))) setSecureCookie("lista_precio", 1, $expire);
...
if ($_GET && isset($_GET["usuario"]) && isset($_GET["clave"])){
    // NOTA: Login via GET no es recomendado - las credenciales quedan en logs
    // Considerar migrar a POST exclusivamente
    header('Content-Type: application/json');

    // Usar prepared statements
    $stmt = $conn->prepare("SELECT id, sucursal_id FROM usuarios WHERE usuario = ? AND clave = ?");
    $clave_hash = sha1($conn->real_escape_string($_GET["clave"]).SEMILLA);
    ...
    if ($resultado_stock->num_rows > 0) {
        if ($row_stock = $resultado_stock->fetch_assoc()) {
            ...
            echo json_encode([
                'token' => $token,
                'sucursal' => $row_stock["sucursal_id"]
            ]);
        }

    }else{
        http_response_code(401);
        echo json_encode(['error' => 'Usuario y/o Clave incorrectos']);
    }
    $stmt->close();
    exit();
}
