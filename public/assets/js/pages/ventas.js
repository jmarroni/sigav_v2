    ...
    jQuery("document").ready(function(){
        ...
        jQuery("#concretar_venta").click(function(){    
            ...
            // Si emitir_factura_valor = 1, presupuesto=0 (factura), sino presupuesto=1 (presupuesto)
            var emitir_factura = $("#emitir_factura_valor").val();
            var es_presupuesto = (emitir_factura == "1") ? "0" : "1";

            $.ajax({
                method: "GET",
                url: "facturar.php?tipo=" + medio_de_pago +
                "&presupuesto=" + es_presupuesto + "&nombre=" + $("#nombre-cliente").val() +
            ...
            .done(function (msg) {
                if (msg.factura){
                    ...
                    var tipo_doc = (es_presupuesto == "0") ? "factura" : "presupuesto";
                    alert('Sucedió un error al generar el ' + tipo_doc + '. ' + (msg.mensaje || msg.error || '') + '. Recargaremos la web.');
    ...
    $("#emision_online").change(function(){
        ...
        var mensaje = emitir
            ? '¿Activar emisión de FACTURA ELECTRÓNICA?\n\nLas ventas se enviarán a AFIP.'
            : '¿Cambiar a modo PRESUPUESTO?\n\nLas ventas NO se enviarán a AFIP.';
