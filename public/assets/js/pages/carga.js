...
function modificar(identificador){
    if ($("#stock_" + identificador).val() < 0 || $("#stock_" + identificador).val() == ''){
        ...
        $.get("/carga/" + identificador, function(data, status){
            ...
            // Scroll suave hacia el formulario de edicion
            $('html, body').animate({
                scrollTop: $("#block-content").offset().top - 100
            }, 500);

            // Resaltar el formulario brevemente
            $("#block-content").css("background-color", "#fffde7");
            setTimeout(function(){
                $("#block-content").css("background-color", "");
            }, 1500);
        });
    }
}
