...
//email body content
//email body content
$htmlContent = file_get_contents(__DIR__ . "/mail/template.php");
$htmlContent = str_replace("@@LOGO@@", "http://".$_SERVER["HTTP_HOST"].$perfil["logo"], $htmlContent);
$htmlContent = str_replace("@@TITULO@@", "Te enviamos la factura por tu compra.", $htmlContent);
$htmlContent = str_replace("@@CONTENIDO@@", 'Desde el siguiente <a href="http://'.$_SERVER["HTTP_HOST"].$file.'" target="_blank">link</a> podes visualizar (o descargar) la factura enviada por '.$perfil["nombre"].'<br />Saludos y gracias por tu compra.<br />'.$perfil["nombre"], $htmlContent);
