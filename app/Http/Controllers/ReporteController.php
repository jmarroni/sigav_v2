...
class ReporteController extends Controller
{
  ...
  public function factura(Request $request, $reporte_desde = null, $reporte_hasta = null){
    ...
    $query = Factura::leftjoin("sucursales","sucursales.id","=","factura.sucursal_id")
      ->where("factura.cae","<>","")
      ->where("factura.fechacae","<>","")
      ->select("factura.*","sucursales.nombre as nombre_sucursal");

    // Si el rol es vendedor (1), solo ver facturas de su sucursal
    if (Rol::getRol() == 1) {
      $query->where("sucursales.id", "=", Sucursales::getSucursal($_COOKIE["sucursal"]));
    }

    // Aplicar filtros
    if ($request->filled('reporte_desde')) {
      $query->whereDate('factura.fecha', '>=', $request->reporte_desde);
    }

    if ($request->filled('reporte_hasta')) {
      $query->whereDate('factura.fecha', '<=', $request->reporte_hasta);
    }

    if ($request->filled('sucursal_id')) {
      $query->where('factura.sucursal_id', $request->sucursal_id);
    }

    if ($request->filled('usuario')) {
      $query->where('factura.usuario', 'like', '%' . $request->usuario . '%');
    }

    if ($request->filled('numero')) {
      $query->where('factura.numero', 'like', '%' . $request->numero . '%');
    }

    if ($request->filled('cae')) {
      $query->where('factura.cae', 'like', '%' . $request->cae . '%');
    }

    if ($request->filled('cliente')) {
      $query->where(function($q) use ($request) {
        $q->where('factura.cliente_nombre', 'like', '%' . $request->cliente . '%')
          ->orWhere('factura.cliente_cuit', 'like', '%' . $request->cliente . '%');
      });
    }

    // Ordenamiento
    $orderBy = $request->get('order_by', 'factura.fecha');
    $orderDir = $request->get('order_dir', 'desc');

    $allowedColumns = ['factura.id', 'factura.fecha', 'factura.numero', 'factura.total', 'factura.usuario', 'sucursales.nombre'];
    ...
    $facturas = $query->paginate(50)->withQueryString();

    // Marcar sucursales eliminadas
    ...
    return view("reportes.factura", compact("facturas", "reporte_desde", "reporte_hasta", "sucursales"));
}

public function facturaHomologacion(Request $request){
    // Obtener sucursales para el filtro
    $sucursales = Sucursales::orderBy('nombre')->get();

    // Construir query base - SOLO facturas de homologacion
    $query = Factura::leftjoin("sucursales","sucursales.id","=","factura.sucursal_id")
      ->where("factura.homologacion", "=", 1)
      ->select("factura.*","sucursales.nombre as nombre_sucursal");
    ...
    return view("reportes.facturaHomologacion", compact("facturas", "sucursales"));
}
...
public function logProductos(Request $request)
{
  ...
  $query = Stock_log::leftjoin("sucursales","sucursales.id","=","stock_logs.sucursal_id")
  ...
  // Aplicar filtros
  if ($request->filled('sucursal_id')) {
    $query->where('stock_logs.sucursal_id', $request->sucursal_id);
  }

  if ($request->filled('usuario')) {
    $query->where('stock_logs.usuario', 'like', '%' . $request->usuario . '%');
  }

  if ($request->filled('tipo_operacion')) {
    $query->where('stock_logs.tipo_operacion', $request->tipo_operacion);
  }

  if ($request->filled('fecha_desde')) {
    $query->whereDate('stock_logs.created_at', '>=', $request->fecha_desde);
  }

  if ($request->filled('fecha_hasta')) {
    $query->whereDate('stock_logs.created_at', '<=', $request->fecha_hasta);
  }
  ...
  if ($request->filled('buscar')) {
    ...
    $query->where(function($q) use ($buscar) {
      $q->where('productos.nombre', 'like', '%' . $buscar . '%')
        ...
        ->orWhere('stock_logs.barra', 'like', '%' . $buscar . '%');
    });
  }

  // Ordenamiento
  $orderBy = $request->get('order_by', 'stock_logs.id');
  $orderDir = $request->get('order_dir', 'desc');

  // Validar columnas permitidas para ordenar
  $allowedColumns = ['stock_logs.id', 'stock_logs.created_at', 'productos.nombre', 'sucursales.nombre', 'stock_logs.usuario', 'stock_logs.tipo_operacion'];
  if (!in_array($orderBy, $allowedColumns)) {
    $orderBy = 'stock_logs.id';
  }
  ...
  // Tipos de operacion para el filtro
  $tiposOperacion = Stock_log::select('tipo_operacion')->distinct()->pluck('tipo_operacion');

  return view("reportes.logsProductos", compact("productos", "sucursales", "tiposOperacion"));
}
